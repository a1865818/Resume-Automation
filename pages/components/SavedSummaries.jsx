...
const SavedSummaries = () => {
  ...
  const deleteSummary = async (summary) => {
    ...
    if (!fileName) {
      setError("Cannot delete: missing file identifier");
      console.error(
        "Attempted to delete summary without fileName or id:",
        summary
      );
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        try {
          const errorData = await response.json();
          throw new Error(
            `Failed to delete summary: ${response.status} - ${
      ...
      console.log("Successfully deleted summary");
      setSummaries(
        summaries.filter((s) => s.fileName !== fileName && s.id !== fileName)
      );
      setTotalCount((prev) => prev - 1);
      setFilteredCount((prev) => prev - 1);
    } catch (err) {
...
// Modify the viewAsPdf function in your SavedSummaries.jsx:
const viewAsPdf = (summary) => {
    const fileName = summary.fileName || summary.id;
    if (!fileName) {
      setError("Cannot view summary: missing file identifier");
      return;
    }
  
    // Use the proper file format - ensure we're referencing the txt file if available
    const fileNameForPdf = fileName.endsWith('.json') ? fileName.replace('.json', '.txt') : fileName;
    
    console.log(`Opening PDF view for: ${fileNameForPdf}`);
    
    // Open in a new window and handle potential errors
    try {
      window.open(`/api/viewAsPdf?fileName=${encodeURIComponent(fileNameForPdf)}`, '_blank');
    } catch (err) {
  ...
  const viewResumeData = async (summary) => {
    const fileName = summary.fileName || summary.id;
    if (!fileName) {
      setError("Cannot view resume: missing file identifier");
      return;
    }

    try {
      // Try to fetch the JSON version first
      const jsonFileName = fileName.replace(".txt", ".json");
      const response = await fetch(`/uploads/summaries/${jsonFileName}`);
      ...
      if (response.ok) {
        ...
        // Fallback to basic info from summary list
        setSelectedResume({
          candidateName: summary.candidateName || "Unknown Candidate",
          jobTitle: summary.jobTitle || "",
          yearsOfExperience: summary.yearsOfExperience || 0,
          ...
          summary: "Detailed resume data not available for this entry.",
          workExperience: [],
          educationDetails: [],
          keyAchievements: [],
          certifications: [],
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold">Saved Resume Summaries</h2>
          <span className="text-gray-500 text-sm">
            {Object.values(filters).some((v) => v)
              ? `Showing ${filteredCount} of ${totalCount} summaries`
              : `${totalCount} ${totalCount === 1 ? "summary" : "summaries"}`}
        ...
        {summaries.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {Object.values(filters).some((v) => v)
                ? "Try adjusting your filter criteria"
                : "Generate and save a resume to see it listed here."}
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            {summaries.map((summary) => (
              <div
                key={
                  summary.fileName ||
                  summary.id ||
                  `summary-${summary.createdAt}-${Math.random()}`
                }
                className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition-colors"
              >
                <div className="flex flex-col md:flex-row md:justify-between md:items-start">
                  <div className="mb-4 md:mb-0 md:mr-4 flex-grow">
                    <div className="flex items-start">
                      ...
                      {summary.jobTitle &&
                        summary.jobTitle !== summary.candidateName && (
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {summary.seniorityLevel && (
                        <span
                          className={`inline-flex items-center rounded-full ${
                            summary.seniorityLevel === "Senior"
                              ? "bg-purple-100 text-purple-800"
                              : summary.seniorityLevel === "Mid-level"
                      ...
                      {summary.yearsOfExperience > 0 && (
                        <span className="inline-flex items-center rounded-full bg-gray-100 text-gray-800 px-2.5 py-0.5 text-xs font-medium">
                          ...
                          {summary.yearsOfExperience === 1 ? "year" : "years"}
                        </span>
                      )}
                  ...
                  <div className="flex flex-row md:flex-col space-x-2 md:space-x-0 md:space-y-2">
                    <button
                      onClick={() => viewResumeData(summary)}
                      className="text-indigo-600 hover:text-indigo-800 text-sm font-medium flex items-center"
                    >
                      <svg
