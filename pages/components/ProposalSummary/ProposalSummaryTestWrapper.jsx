          ...
          new Paragraph({
            children: [
              new ImageRun({
                ...
                transformation: {
                  width: 623, // Matching your banner width
...
const ProposalSummaryWrapper = ({ 
  ...
  // LEGACY: Handle Word download with HTML approach (CREATES FOLDERS - kept for compatibility)
  const handleDownloadWordLegacy = async () => {
    setIsWordLoading(true);
    ...
    try {
      ...
      // Enhanced filename
      const filename = `${sanitizedName}_${detectedSector}_Proposal_Summary_Legacy.doc`;
      ...
      console.log(`${detectedSector} Proposal Summary Legacy Word document generated successfully!`);
    } catch (error) {
  ...
  return (
    <div style={{ 
      ...
      <div style={{
        ...
        {/* OPTIONAL: Legacy Word Download Button - CREATES FOLDERS */}
        {/* Uncomment if you want to keep both options
        <button
          onClick={handleDownloadWordLegacy}
          disabled={isWordLoading || isRegenerating}
          className={`px-4 py-2 rounded-lg font-medium flex items-center gap-2 transition-colors duration-200 text-sm ${
            isWordLoading || isRegenerating
